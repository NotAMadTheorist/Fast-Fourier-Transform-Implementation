...
print(f"Suppose you want to multiply the two polynomials and get their coefficients:\n"
      f"P1(x) = 3 - x + 3x^2 - 7x^3 + 0x^4 + 6x^5 + 7x^6 + 7x^7\n"
      f"P2(x) = 5 - 3x - 3x^2 + 7x^3 - 5x^4 + x^5 + 0x^6 - x^7\n\n"
      f"Let the following list of values represent their coefficients:\n"
      f"P1 = [3, -1, 3, -7, 0, 6, 7, 7]\n"
      f"P2 = [5, -3, -3, 7, -5, 1, 0, -1]\n\n")
...
print("\n Note that the FFT-IFFT algorithm is able to compute the coefficients of the polynomial product much\n"
      "faster than the lattice method.")
input("")
...
# the same operations on the numbers themselves, how would we interpret the output?
...
legend_positions = ["upper right", "lower right", "upper left", "lower left", "upper left", "lower center"]
legend_index = 0
for F1, F2 in functions:
    ...
    fig.suptitle("Plot of Scaled Convolution Function (Y1 ⨂ Y2)(x)")
    axis[0].plot(xRange, y1Range, 'tab:green', label="Stationary Function Y1(x)")
    axis[0].plot(xRange, y2Range, 'tab:orange', label="Sliding Function Y2(x)")
    ...
    axis[1].plot(xRange, ycRange, 'tab:blue', label="Convolution (Y1 ⨂ Y2)(x)")
    axis[1].set(xlabel = 'x', ylabel = '(Y1 ⨂ Y2)(x)')
    axis[1].legend(loc=legend_positions[legend_index])
    plt.show()
    legend_index += 1
