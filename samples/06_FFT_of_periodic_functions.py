...
# Results involving the DFT suggest the following conjecture:
...
#
# This above principle enables such periodic functions to be approximated as a sum of sine and cosine functions
# at a discrete set of frequencies. This is also known as the Fourier Series.
...
waves = [('Constant Function', lambda x: constant(x, 5)),
         ('Periodic Parabola Function', lambda x: f_parabola(x, 15)),
         ('Periodic Cubic Function', lambda x: f_cubic(x, 15)),
         ('Periodic Exponential * Sine Function', lambda x: f_exposin(x, 15)),
         ('Periodic Square + Sawtooth Function', lambda x: f_nessound(x, 15))]
...
for waveName, waveFunc in waves:
    ...
    uSpikes = [float(round(point[1], 3)) for point in spikePoints]
    ...
    fig.suptitle(f'Plot of {waveName} and its DFT')
