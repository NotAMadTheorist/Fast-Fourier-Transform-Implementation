...
from fast_fourier_transform import FFT, pad_zeros, nearest_power_of_2, waveform_square
...
for name, signalY in signals:
    for samplingRangeX in [1, 3, 4]:
        ...
        fig.suptitle(f"Discrete Fourier Transform of a {name} from x = 0 to x = {samplingRangeX}\n"
                     f"Frequency from f = 0 Hz to Nyquist f = {nyquistF} Hz")
...
print("Only the heights of the spikes seem to change as we increase the range of x; as the range of x increases, \n"
      "the heights of the spikes become larger, but the overall distribution of frequencies still stays the same.")
input("")
